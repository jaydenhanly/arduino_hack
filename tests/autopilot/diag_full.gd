...
func _ready() -> void:
	...
	report("A_state_before_confirm", game.state)
	report("A_stage_before_confirm", game.current_stage)
	report("A_score_before_confirm", game.score)
	report("A_lives_before_confirm", game.lives)
	await press("confirm", 20)
	report("A_won", won)
	report("A_state_after", game.state)
	report("A_stage_after", game.current_stage)
	report("A_score_after", game.score)
	report("A_lives_after", game.lives)
...
func _test_checkpoints() -> void:
	...
	report("B_sel_stage", game.current_stage)
	report("B_sel_state", game.state)
	report("B_sel_apples", game.stage.apples)
	report("B_sel_err", game.playtest.last_error)
	report("B_sel_bodylen", game.stage.body.size())
	report("B_sel_walls", game.stage.walls.size())
	report("B_dev_active", game.playtest.active)
	report("B_panel_open", game.playtest.panel_open)
	await press("dev_complete", 20)
	report("B_after_state", game.state)
	report("B_after_stage", game.current_stage)
	report("B_after_apples", game.stage.apples if game.current_stage == "snake" else -1)
	report("B_after_err", game.playtest.last_error)
	report("B_after_shift_elapsed", game.shift_elapsed)
