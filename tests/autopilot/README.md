...
## Pixel Shift acceptance probes

The default `run.sh` target is now `pixel_shift_probe.gd`. It plays from the title through Snake, the visible shift, Maze victory, and replay, then checks collision fixtures and development presets. Run `snake_probe.gd`, `checkpoint_probe.gd`, or `variation_probe.gd` explicitly for the narrower suites. See `../../README.md` for commands.

The runner checks engine exit status, reported failures, frame warnings, and timeout results. Set `OUT_DIR` to retain separate evidence from multiple runs. Python 3 is required to validate results.

## Original waypoint template

The remaining guide describes `autopilot.gd`, which is retained as a starter template rather than used by the current acceptance checks.
