...
func _run() -> void:
	...
	check("periodic comments never vibrate", feedback.last_event.packet.vibration.is_empty())
	feedback.emit_feedback("checkpoint")
	check("checkpoint does not create duplicate haptics", feedback.last_event.kind == "comment")
	feedback.emit_feedback("collect")
	check("feedback has a rate limit", feedback.last_event.kind == "comment")
	feedback.advance(1.0)
	feedback.emit_feedback("victory", 2.0)
	feedback.advance(0.1)
	check("feedback does not expose exact progress", not feedback.last_event.packet.has("progress"))
	check("matrix remains optional", feedback.last_event.packet.frames[0].rows.size() == 8)
	check("terminal event replaces collect", feedback.last_event.kind == "victory")
	feedback.free()
	...
	for profile in ["demo", "normal"]:
		...
		game.hardware.advance(0.1)
		check(profile + " transform vibrates (not swallowed by the checkpoint cue)", game.hardware.last_event.kind == "transform" and not game.hardware.last_event.packet.vibration.is_empty())
	...
	game.hardware.advance(0.1)
	check("death vibrates (not swallowed by the checkpoint cue)", game.hardware.last_event.kind == "death" and not game.hardware.last_event.packet.vibration.is_empty())
	...
	game.hardware.advance(0.1)
	check("victory vibrates (not swallowed by the checkpoint cue)", game.hardware.last_event.kind == "victory" and not game.hardware.last_event.packet.vibration.is_empty())
	game.start_run(77)
